...
const ClockHeatmap = ({
  ...
  const drawHeatmap = () => {
    ...
    if (isZoomed) {
      ...
      ctx.fillRect(width - 110, 10, 100, 110);
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (e.clientX > width - 110 && e.clientX < width - 10 && e.clientY < 110) {
      return;
    }
  ...
  const handleWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    const newZoomLevel = Math.max(5, Math.min(MAX_ZOOM_LEVEL, zoomLevel + (e.deltaY > 0 ? -2 : 2)));
    setZoomLevel(newZoomLevel);
  };
  ...
  return (
    <div className="relative">
      <canvas
        ...
        width={width}
        height={height}
        className={`rounded-lg shadow-lg ${isZoomed ? 'cursor-crosshair' : 'cursor-pointer'}`}
